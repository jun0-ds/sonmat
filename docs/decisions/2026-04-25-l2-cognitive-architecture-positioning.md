...
OpenRabbit AI 사업 인계문(2026-04-25, `~/.claude/memory/domain/openrabbit.md`)에서 빌드 표준 4층 위계가 정리됨:
...
이 위계에서 **sonmat의 현재 위치**가 명시됨: "L2 인지 아키텍처의 개인용 R&D 프로토타입". 같은 OpenRabbit 4층 위계 안에서 sonmat과 OpenRabbit 사내 IP는 **같은 L2를 공유**, 채널만 다름:
- sonmat = 개인 세팅 → 범용 → 오픈소스 채널
- OpenRabbit = 상용·사내 IP 채널
...
4. **회수 파이프라인 가능 상태 유지**: sonmat에서 검증된 패턴이 OpenRabbit L2 사내 자산으로 일반화 가능하도록 추상화 강도를 높인다. 라이센싱·brand는 채널별로 분기되더라도 L2 IP는 공유.
...
- **OpenRabbit 회수 가능**: sonmat에서 검증된 패턴이 OpenRabbit L2 사내 IP로 일반화 가능. 두 채널이 같은 IP 공유
...
- **두 채널 IP 충돌 가능성**: sonmat 오픈소스 + OpenRabbit 상용이 같은 L2 공유. brand·라이센싱·기능 포지셔닝 충돌 방지 정책은 후속 ADR로

### 트레이드오프 검증 시점

- 다음 sonmat 버전(현재 v0.9.x → v1.0) 릴리스 시 본 결정의 정합성 재검토
- OpenRabbit 첫 L2 자산 회수 시도 시 본 결정의 추상화 강도가 충분했는지 평가
...
- `2026-XX-XX-recovery-pipeline.md` — sonmat → OpenRabbit L2 자산 회수 절차

## 참조

- `~/.claude/memory/domain/openrabbit.md` — 4층 위계 본체 + sonmat 위치 명시
- `~/Documents/001_OpenRabbit/docs/decisions/2026-04-25-agent-shell-folder-core.md` — OpenRabbit 첫 ADR (보강 절에서 4층 위계 명시)
